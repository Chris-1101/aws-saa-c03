...
echo -e "${efs_filesystem_id}:/ /var/www/html/wp-content efs _netdev,tls,iam 0 0" >> /etc/fstab
...
sed -i "s/'localhost'/'${rds_hostname}'/g" wp-config.php
...
/opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl -a fetch-config -m ec2 -c ssm:${cw_config_param} -s
